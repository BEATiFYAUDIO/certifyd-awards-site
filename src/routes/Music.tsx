...
import { categories, entries, getCategory } from '../data/awards';
...
export function Music() {
  ...
  return (
    <section className="page-section awards-division-page">
      ...
      <section className="content-section tight"><div className="section-heading"><span className="eyebrow">Creative categories</span><h2>Work, stories, performance, and community impact.</h2></div><div className="category-strip">{creativeCategories.map((category) => <Link className="large-category-tile" to={`/music/categories/${category.slug}`} key={category.id}><h3>{category.title}</h3><p>{category.summary}</p></Link>)}</div></section>
