    ...
    proofs: [
      {
        ...
        source: String(item.publicOrigin || ''),
      },
    ],
    scoring: [
      { label: 'Support', reason: 'Public support, purchase, unlock, tip, or popularity signal.', normalizedScore: supportNormalized, weight: 0.35 },
      ...
      { label: 'Availability', reason: 'Reachable public item with commerce or healthy origin signals.', normalizedScore: postureNormalized, weight: 0.15 },
    ],
    fanSupportSats: 0,
    ...
    .map(([origin, rows]) => {
      ...
      return {
        ...
        source: {
          ...
          period: 'Current',
          methodology: `${creators.size} creator ${creators.size === 1 ? 'profile is' : 'profiles are'} currently returning work from this origin. Operator readiness belongs on the Certifyd Network map.`,
        },
        __score: score,
