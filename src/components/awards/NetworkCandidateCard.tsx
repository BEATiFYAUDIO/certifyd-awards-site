  ...
  return (
    <article className="network-profile-card">
      ...
      <div className="network-profile-main">
        <div className="network-profile-avatar" aria-hidden="true">{initialsFor(ranking.title)}</div>
        <div>
          <h3>{ranking.title}</h3>
      ...
      <div className="network-profile-stats">
        <span><strong>{profile.roles.length}</strong> roles</span>
        <span><strong>{profile.proofCount}</strong> proofs</span>
        <span><strong>{profile.nodeAgeDays ?? '—'}</strong> days</span>
      </div>
